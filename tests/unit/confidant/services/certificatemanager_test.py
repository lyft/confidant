...
@pytest.fixture()
def ca_object(mocker: MockerFixture) -> certificatemanager.CertificateAuthority:
...
def test_generate_key(ca_object: certificatemanager.CertificateAuthority):
...
def test_encode_key(ca_object: certificatemanager.CertificateAuthority):
    key = ca_object.generate_key()
    encoded_key = ca_object.encode_key(key)
    assert encoded_key.startswith('-----BEGIN RSA PRIVATE KEY-----')


def test_generate_x509_name(ca_object: certificatemanager.CertificateAuthority):
...
def test_generate_csr(ca_object: certificatemanager.CertificateAuthority):
    key = ca_object.generate_key()
...
def test_encode_csr(ca_object: certificatemanager.CertificateAuthority):
    key = ca_object.generate_key()
...
def test_decode_csr(ca_object: certificatemanager.CertificateAuthority):
...
def test_get_csr_san(ca_object: certificatemanager.CertificateAuthority):
    key = ca_object.generate_key()
...
def test_encode_certificate(ca_object: certificatemanager.CertificateAuthority):
    key = ca_object.generate_key()
    cert = ca_object.generate_self_signed_certificate(
        key,
        'test.example.com',
...
def test__get_cached_certificate_with_key(
    ...
    mocker.patch(
        'confidant.services.certificatemanager.CertificateCache.get',
        return_value=item
    )
...
def test_issue_certificate_with_key(
    ...
    mocker.patch(
        'confidant.services.certificatemanager.CertificateAuthority._get_cached_certificate_with_key',  # noqa:E501
