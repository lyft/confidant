...
def _handleUpdateException(e, item):
    ...
    if code == 'ConditionalCheckFailedException':
        app.logger.warn('conditional update failed (concurrent writes?) for object:',
...
def migrate_boolean_attributes(model_class,
    ...
    :param allow_scan_without_rcu: Passed along to `rate_limited_scan`; intended
    ...
    :param max_items_updated_per_second: An upper limit on the rate of items update per second.
    :return: (number_of_items_in_need_of_update, number_of_them_that_failed_due_to_conditional_update)
    """
    app.logger.info('migrating items; no progress will be reported until completed; this may take a while')
    ...
    for item in model_class.rate_limited_scan(_build_lba_filter_condition(attribute_names),
                                              ...
                                              page_size=page_size,
                                              limit=limit,
                                              allow_rate_limited_scan_without_consumed_capacity=allow_scan_without_rcu):
        items_processed += 1
        if items_processed % 1000 == 0:
            app.logger.info('processed items: %s Thousand', items_processed/1000)
    ...
    app.logger.info('finished migrating; %s items required updates',
             num_items_with_actions)
    app.logger.info('%s items failed due to racing writes and/or exceeding capacity and require re-running migration',
             num_update_failures)
...
class MigrateBooleanAttribute(Command):
    ...
    option_list = (
        Option('--model', action="store", dest="model_name", type=str, required=True,
               help='The model that should be migrated. Choose from service, blind_credential or credential'),
        Option('--RCU', action="store", dest="RCU", type=int,
               default=10, help='Read Capacity Units to be used for scan method.'),
        Option('--page-size', action="store", dest="page_size", type=int,
               default=None, help='Page size used in the scan.'),
        Option('--limit', action="store", dest="limit", type=int,
               default=None, help='Limit the number of results returned in the scan.'),
        Option('--back-off', action="store", dest="back_off", type=int,
               default=1, help='Number of seconds to sleep when exceeding capacity.'),
        Option('--update-rate', action="store", dest="update_rate", type=float,
               default=1.0, help='An upper limit on the rate of items update per second.')
    )

    def run(self, model_name, RCU, page_size, limit, back_off, update_rate):
        attributes = ['enabled']
        app.logger.info('RCU: {}, Page Size: {}, Limit: {}, Back off: {}, '
                        'Max update reate: {}, Attributes: {}'.format(
